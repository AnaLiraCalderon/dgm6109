...
//--------------------------------------------//
/* LOADING OF DATA BASE IN THE JSON AND FLOW  */ //Source: Code structure adapted from Professor Alfredo, Lab #9
//--------------------------------------------//

/* This self-invoking function serves as the initializer for the program’s execution flow. It utilizes an 
asynchronous structure to fetch the "data.json" file and, upon a successful retrieval, passes the resulting 
dataset to the simsGamingObservations function for further processing. Once the data is handed off, 
it logs a confirmation message to the console to verify that the loading phase is complete. */
...
//This function acts as a coordinator that manages the data flow between the retrieval and 
// the final rendering of the project. It first receives the raw data from the JSON file and 
// passes it to the organizeData function to be filtered and structured. Once the information 
// is processed, it invokes the drawVisualization function to generate the visual elements 
// on the SVG canvas. Finally, it returns the original dataset to ensure it remains 
// accessible for any subsequent operations within the program.
...
//--------------------------------------//
/*-----  CREATE A SCALE AND FUNTIONS---*/ //Source: Code structure adapted from Professor Alfredo, Lab #9
//--------------------------------------//

/*----- ORGANIZE DATA: MULTI-FILTER -----*/

/* This function is responsible for filtering the dataset to ensure that only valid entries are used for the visualization. 
It utilizes the JavaScript filter method to check each day's record, returning only those where essential properties, such 
as the date, time of day, temperature, playtime minutes, and stress levels, are not undefined. By performing this validation, 
the function prevents errors during the rendering process and guarantees that the final calendar only displays complete observations. */
...
//--------------------------------------------//
//This function draw the information into the grid space.
...
/* This code segment utilizes a forEach loop to iterate through the dataset, processing each individual entry one by one to determine 
its precise location on the canvas. It calculates the grid position by assigning each data point to a specific column and row based 
on its index, effectively organizing the observations into a weekly calendar structure. By defining the horizontal and vertical 
coordinates (xPos and yBase), the script ensures that each day is spaced consistently, preventing overlaps and creating a clear, structured 
layout for the final visualization. */
...
let timeOfDay = svg.append("g")
    .attr("transform", "translate(150, 50)"); /* TECHNICAL REFLECTION (Lab 8):
...
let temperature = svg.append("g")
    .attr("transform", "translate(400, 50)"); /* TECHNICAL REFLECTION (Lab 8):
I learned how to use the group element <g> in D3 to encapsulate all the elements 
of a legend (circles and text). This allowed me to move the entire legend as a 
single unit using a single line of code (transform, translate), instead of 
calculating the position of each individual piece relative to the chart.
...
let stressLevelAfter = svg.append("g")
    .attr("transform", "translate(640, 50)"); /* TECHNICAL REFLECTION (Lab 8):
I learned how to use the group element <g> in D3 to encapsulate all the elements 
of a legend (circles and text). This allowed me to move the entire legend as a 
single unit using a single line of code (transform, translate), instead of 
calculating the position of each individual piece relative to the chart.
