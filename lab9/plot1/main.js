...
let sortByStress = function (a, b) {
    return b.stressLevelAfter - a.stressLevelAfter; 
}
//--------------------------------------------------------
...
let ByStress = svg
    ...
    .data(simsGamingObservations.filter(filterTimePlayed).sort(sortByStress)) //this is the filter and the sort
    .join("circle")
    .classed("data-point", true)
    .attr("cx", function (value) {
        return xScale(value.temperatureC);
    })
    .attr("cy", function (value) {
        return yScale(value.stressLevelAfter); 
    })
    .attr("r", function (value) {
        return rScale(value.minutesPlayed); 
    })
    .attr("opacity", 0.7)
    .attr("stroke", "black")
